...
if ( ! class_exists( 'Bxcft_Plugin' ) ) {
	class Bxcft_Plugin {
		...
		public function init() {
			$this->images_ext_allowed       = apply_filters(
				'bxcft_images_ext_allowed', array(
					'jpg',
					'jpeg',
					'gif',
					'png',
			...
			$this->files_ext_allowed        = apply_filters(
				'bxcft_files_ext_allowed', array(
					'doc',
					'docx',
					'pdf',
				)
			...
			$this->fields_type_with_select2 = apply_filters(
				'bxcft_field_types_with_select2', array(
					'selectbox',
					'multiselectbox',
			...
			$this->fields_type_multiple     = apply_filters(
				'bxcft_field_types_multiple', array(
					'multiselectbox',
					'multiselect_custom_post_type',
					'multiselect_custom_taxonomy',
				)
		...
		public function bxcft_get_field_data( $value, $field_id ) {
			...
			return apply_filters(
				'bxcft_show_field_value', $value, $field->type,
				$field_id, $value
			);
		}
		...
		public function bxcft_profile_upload_dir( $user_id = 0 ) {
			 ...
			 * @var string
			 */
			$profile_subdir = apply_filters(
				'bxcft_upload_dir', '/profiles/' . $this->user_id,
				$this->user_id
			);
		...
		?>
		<div id="select2-box" class="postbox
		<?php
		if ( $hidden ) :
?>
 hidden<?php endif; ?>">
			...
			<div class="inside">
				...
				<p>
					...
					<select name="do_select2" id="do-select2">
						...
						if ( $do_select2 === 'on' ) :
?>
 selected="selected"<?php endif; ?>>Enabled</option>
						<option value=""
						<?php
						if ( $do_select2 !== 'on' ) :
?>
 selected="selected"<?php endif; ?>>Disabled</option>
					</select>
				</p>
			</div>
		</div>
