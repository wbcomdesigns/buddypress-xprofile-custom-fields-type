...
if ( ! class_exists( 'Bxcft_Field_Type_Birthdate' ) ) {
	class Bxcft_Field_Type_Birthdate extends BP_XProfile_Field_Type {
		...
		public function admin_field_html( array $raw_properties = array() ) {
			$day_html = $this->get_edit_field_html_elements(
				array_merge(
					array(
			...
			$month_html = $this->get_edit_field_html_elements(
				array_merge(
					array(
			...
			$year_html = $this->get_edit_field_html_elements(
				array_merge(
					array(
				...
				<div class="inside">
					<p>
						...
						<input type="checkbox"
							...
							<?php
							if ( $options[0]->name == Bxcft_Field_Type_Birthdate::OPTION_SHOW_AGE ) :
?>
			...
			if ( ! empty( $field_value ) && ! empty( $field_id ) ) {
				...
				if ( $field ) {
					...
					if ( $show_age ) {
						...
						$new_field_value = date_i18n(
							get_option( 'date_format' ),
							strtotime( $field_value )
						);
					}

					$do_autolink = apply_filters(
						'bxcft_do_autolink',
					...
					if ( $do_autolink ) {
						...
						$search_url      = add_query_arg(
							array( $query_arg => urlencode( $field_value ) ),
